...
| 消费者 | 在哪步用本文档 | 用法 |
|---|---|---|
| **author agent**(Stage C-D) | 设计 outline + 拓写 content | 按 13 layout 文案规则写;按 Pyramid 5 件套设计 outline;按 markdown schema 输出 .md;**每个 `## N.` 内容页强制 `<!-- layout: X -->` 注释** |
| **iloveppt-builder agent**(Stage E build) | 拿到 content.md → md→JSON | strict 1:1 解析:缺 `<!-- layout: X -->` → hard_stop,不做结构推断 |
...
> 单点收口在 **critic**(Section A 7 项金字塔审计):author 按 Pyramid 5 件套设计 outline;critic Stage C/D 是唯一判定 gate;iloveppt-builder 不重跑。author state file 含 `top_recommendation` / `scqa` 字段,critic 拿来逐项验证。
...
**违规处理**(author Stage C + iloveppt-builder Stage E 都要检查):
...
章节标题串起来 = 顶端论点的完整论据链。把所有章节 + 页面 action_title 抽出来读一遍 —— 应当读出一个**自洽的故事**(ghost deck test,critic Section A 维度 6)。

---

### Pyramid 7 项审计表(critic Stage C/D Section A 跑)

> 这张表**只在 critic** 跑一次,作为 Pyramid 单点判定。author 按表设计 outline 即可,**不自跑** 7 项 checkbox;iloveppt-builder 不重跑。
...
critic 任一项 fail → `verdict: needs_revision`,主线程让用户 cherry-pick 改动指令 → Task author rework。多项 fail 时 author 一次改 must-fix,不接受"先放着"含糊。

### 纯展示场景

下列场景**不是论证型 deck**,7 项审计会有部分项不适用:

- **纯数据汇报 / 月报 / 周报**:章节是数据维度而非论证
- **培训 / 教学 deck**:目标是讲解知识结构,不是说服决策
- **目录式索引 deck**:产品手册、能力清单、人员介绍等

action title ≤ 24 字硬约束(第 7 项)始终保留(排版工程约束)。critic 评审时,brief.audience / brief.top_recommendation 已暗示场景类型,critic 自动放宽判断尺度;若误判,用户 cherry-pick 拒绝该 issue 即可。
...
- critic Stage C 通过后,`outline` 决定 `deck_plan.json` 的 slides 骨架
...
brief.md schema 见 [iloveppt-brainstorm agent](${CLAUDE_PROJECT_DIR}/.claude/agents/iloveppt-brainstorm.md) "情况 B" 段(brainstorm Step 3 写 brief 的完整模板)。author Stage C/D 在拓写时如下消费这些字段：
...
# (outline.md 末尾不附 Pyramid 自检 checkbox · critic Stage C Section A 判定)
```
...
**强制**:每个 `## N. <action title>` 内容页**必须**紧跟 `<!-- layout: X -->` HTML 注释,X ∈ 13 layouts。
...
特殊 slide(`## [cover]` / `## [toc]` / `## [section_divider]` / `## [summary]` / `## [closing]`)已隐式声明 layout,**无须**额外注释。

**iloveppt-builder 行为**:`## N.` 缺 `<!-- layout: X -->` → 立即返回 `hard_stop: missing_layout_directive`,要求 author 补。不做结构推断。
...
iloveppt-builder 在 md→JSON 转换时**必须强制校验**:字段超限 → Step 3 视觉 QA 阶段在 `deck_plan.json` 截短(机械字数),记录到 `deck_plan_edits[]`;若机械截短会偏移语义 → `review_needed_pages.category: needs_author_rewrite` 升 author。**author/content.md 不可改**。
