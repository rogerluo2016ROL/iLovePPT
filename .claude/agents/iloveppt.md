...
**Step 0.0 · Read critic Stage D 报告(v0.5.1 强前置 gate)**

构建前**必须**先 Read `<working_dir>/critic_report_D.md`:

- 文件不存在 → 立即返回 `error: critic_d_missing` + 提示主线程先派 iloveppt-critic stage=D
...
- **(v0.5.3)pattern 注释处理**:若 page 含 `<!-- pattern: <id> -->` 注释:
  - Read `<iloveppt_root>/library/visual-patterns/patterns/<id>/pattern.yaml`
...
- **必须先 Read critic_report_D.md**(v0.5.1):`verdict == needs_revision` 立即 hard stop;不允许跳过 critic Stage D gate。`pass_with_notes` 视为通过
...
- 不要在 critic Stage D verdict != pass 时硬跑(v0.5.1)——必须 Read critic_report_D.md 验 verdict,needs_revision 返 error(`pass_with_notes` 视为通过)
