...
- **pattern 注释处理**:若 page 含 `<!-- pattern: <full-id> -->` 注释(full-id 形如 `vp:<id>` 或 `tpl:<name>__<NN-slug>`):
  ...
  - 看 `fallback_rendering.method`:
    ...
    - `diagram` → 调 `[[diagram]]` skill 用 draw.io 现画,渲染后 path 写入 deck_plan.json slide 的 `image_path`(走 pic_text 嵌)
...
3. 在 visual_report 同步记录 `rag_fallback_used`(给 audience 看哪些页用了 RAG · 含 `source` 字段标 preferred-template / visual-patterns)
