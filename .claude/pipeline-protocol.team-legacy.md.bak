...
# 然后 SendMessage 给 iloveppt,入参 critic_d_report_path=<latest>
```
...
不传历史问题 —— brainstorm 自己从 `brainstorm/state.json` 捞。state file 是 SSOT,主线程不复述。
...
**Stage C outline.md frontmatter 默认填 footer_meta** — author 出 outline 时默认写入(classification=`INTERNAL` / project=`<deck_slug>` / version=`v1.0`),用户可在审 outline 时改。这避免 brainstorm 多问 3 个低优先级字段;若 author 漏填,iloveppt 透传无值不画 footer(双保险)。
...
## 6. author → iloveppt handoff

**author 窗口保留到 iloveppt Step 0 通过才能关** — author 的工作真正完成的标志,不是"用户批准 content",而是 **iloveppt 跑过 Step 0 Pyramid 自检拿到内容层绿灯**。Step 0 若失败,author 窗口立即被召回,直接 Read state file + content.md 续接修改。Step 0 通过后主线程关闭 author 窗口。

**content.md 不被 iloveppt 改 —— iloveppt 写副本** — iloveppt Step 3.4 的 `auto_md_edits` 不允许写回原文 `deck_v{N}_content.md`,而是写副本 `deck_v{N}_content.postbuild.md`:
- 原文 = 用户批准的最后版本(SSOT,不可变)
- 副本 = iloveppt 自动调整后的实际构建版本(`deck_plan.json` 从副本生成)
- iloveppt 返回主线程时附 `auto_md_edits[]` diff,主线程把"iloveppt 自动调整了这些"展示给用户
- 用户决定:接受副本 → 后续 author 改动以副本为基础;拒绝 → 保留原文,author 后续按用户指令重改
- author 被召回时只 Read 原文 `deck_v{N}_content.md`,不读副本(避免污染)

**iloveppt 入参 `footer_meta` 哪来** — author 在 Stage C `outline.md` frontmatter 里默认填(见上节)→ Stage D 拓写时透传到 `content.md` frontmatter → iloveppt 从 content.md frontmatter 读取。author dispatch_builder 时不重复传 footer_meta,iloveppt 自己 Read content.md 拿。

**iloveppt hard stop 回主线程的处理** — iloveppt Step 0 Pyramid fail 或反向 diff fail 返回 `error: ...` 时,主线程**不自动重派 author**,而是把错误展示给用户,**三选一**:
- "按 iloveppt 的 suggestion 自动改" → 主线程 SendMessage 给 author,把 `failed_items + evidence + suggestion` 当 user_response 传入,author 按 suggestion Edit
- "我来指定怎么改" → 主线程接用户的指令,转发给 author
- "终止" → 主线程关 team,保留 working_dir 现状供后续手动处理

不允许"主线程自动按 suggestion 改"—— iloveppt 的 suggestion 是机械建议(例如"合并章节 2 和 3"),用户可能不同意。
...
主线程派 critic(stage=D)
  ...
  ├── pass / pass_with_notes → 主线程派 iloveppt
...
| verdict | 触发 | 主线程怎么处理 |
|---|---|---|
| `pass` | checklist 全过 + 无 high severity 判断性 | 派下一步(C→author Stage D;D→iloveppt) |
...
| iloveppt Step 0 | 构建前 | 硬阻塞(hard stop) | 进 mechanical build 前最后保险 |
...
**iloveppt Step 0 必须 Read critic_report_D_r{N}.md 确认 pass** — iloveppt 启动时第一动作:
- 文件不存在 → 拒绝构建,return `error: critic_d_missing`
- `verdict == needs_revision` → 拒绝构建,return `error: critic_d_not_passed`
- `verdict == pass` 或 `pass_with_notes` → 继续 iloveppt Step 0 自跑 Pyramid(3 层防线的第 3 层)

---

## 8. iloveppt 内部 + 视觉 QA 三方定位

iloveppt 包含 build + 视觉 双职责。三套视觉 QA 严格分离:
...
**Pyramid 自检 iloveppt 独立跑,但读 author 豁免状态** — iloveppt Step 0 独立跑 7 项自检(基于当前 md 文件状态,含用户手改后版本),不信任 author 的自检结果(author 可能漏标 / 用户后续手改了 md)。但 iloveppt Read author state(`author/state.json`)拿 `pyramid_known_issues`,若 iloveppt 检出 fail 的项恰好是 author 已豁免的,错误信息加注:
...
note: "author 已豁免此项(理由:数据下周才有)。iloveppt 仍判定 fail,需用户最终决定:接受 author 豁免 跳过 / 改 md / 终止"
```

这是 **iloveppt 唯一允许 Read author state 的场景**(handoff 隔离的例外)。

**`deck_plan.json` 持久化 + commit-worthy** — iloveppt 生成 `<working_dir>/builder/deck_plan.json` 到工作目录,**不视为 ephemeral**。用户可绕过 author 直接手改 deck_plan.json 跑 `build.py` 重新出 .pptx(适用 mechanical tweak 场景,如调字号 / 微调位置)。这跟"the seam is deck_plan.json"的设计一致。
...
**5 轮上限 + 用户决定续命** — `audience → author → iloveppt → audience` 这个循环上限 5 轮(主线程在 team 元数据里维护 `audience_round` 计数器)。第 5 轮 audience 仍 < 9 时,主线程**不自动继续**,问用户:
...
│   ├── deck_v{N}_content.postbuild.md        ← iloveppt 自动调整版(副本)
...
**反例 2**:4 轮 audience 把 17 个 issue 全打包 4 选 1 给用户 cherry-pick。**应该按 §13.1 三档 severity 分流(HIGH 自动接 / MED 主线程判 / LOW batch 问)**。
...
| theme code | audience / iloveppt 标了 architectural / theme_fix 项吗?主线程是否该改 themes/<name>.py? |
...
### 13.4 trivial rebuild 模式 · 跳过完整 iloveppt dispatch

content.md 改动 < 10%(< 5 个字段 / < 3 页)或仅是 quick fix(typo / 字数压缩 / markdown literal cleanup),**主线程不派完整 iloveppt**:

```
主线程直接:
1. 跑 `python3 ${CLAUDE_PROJECT_DIR}/.claude/skills/pptx-deck/build.py <working_dir>/builder/deck_plan.json`
...
**违反任一** → 走完整 iloveppt dispatch(含 Step 0 + 36 页 visual QA)。

### 13.5 跨版本 build · 强制 36 页全 Read,禁止 spot check

`content.md → content.md v2`(版本号 +1 或 layout 类型新增 ≥ 1 类)的 build,**iloveppt 必须 36 页全 Read**,**不允许 spot check**。
