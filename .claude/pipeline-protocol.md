...
**P2-3 改动 summary**:
- **P2-3.1** critic Stage B → 并入 brainstorm Step 3.6 self-audit
- **P2-3.2** critic Stage C / Stage D → 合并为 critic stage=cd(author 自走 C→D 无中间 critic)
- **P2-3.3** 主线程 spot-check → 并入 audience Step 0(builder 直接 dispatch_audience)
- 用户 checkpoint 5→3(brief OK / outline OK / content OK + pptx OK 收口)
...
## §0c. ~~主线程 spot-check gate~~(P2-3.3 已并入 audience Step 0)

**P2-3.3 后**:主线程 spot-check 步骤**取消**,完整 spot-check 4 项检查已并入 audience agent 的 Step 0.0:
...
- `critic_report_C_r1.md` / `critic_report_D_r2.md` / `deck_v1_critic_C.r1.md` / `deck_v1_critic_D.r2.md` → P2-3.2 后改为 `deck_v1_critic_cd.r1.md`(C+D 合并)
...
| 3 | outline 已批准 | author return `dispatch_self_stage_d` → 主线程立即 `Task(author, args={stage: "D", outline_md_path: ...})` 续走(P2-3.2 后无中间 critic),产 `author/deck_v1_content.md` | `ask_user_for_content_approval` |
...
| 6 | builder `dispatch_audience` | `Task(audience, args={rendered_dir: "builder/deck_v1_render/", deck_plan_path: ..., builder_visual_edits: [...], audience: <type>, top_recommendation: ..., brief, content_md_path: ..., brief_md_path: ..., report_path: "audience/deck_v1_audience.r{R}.md"})`(P2-3.3 后入参含 spot-check 所需 deck_plan_path + builder_visual_edits + content_md_path + brief_md_path) | `delivered` 或 `needs_*` |
...
用户选叫停 → 主线程 SendMessage 给 brainstorm `{force_dispatch: true}`,brainstorm 用 state 里已有字段 + 默认值组装 brief,**仍跑 Step 3.6 brief self-audit**(默认值组的 brief 更容易踩 B.1-B.5 fail;P2-3.1 后 self-audit 是兜底,不再有外部 critic Stage B)。self-audit 通过后 return `dispatch_author`。
...
用户回 OK 后,brainstorm 下次 SendMessage 返回 `dispatch_author`。**注意**self-audit 5 项(必填字段 / 内部一致性 / theme tier / red_line / top×audience 张力)由 brainstorm 自己跑,不再有 critic Stage B hard gate(P2-3.1 合并)。
...
| content.md 用户审批 | 同上 |
| critic verdict | `pass` 或 `pass_with_notes`(P2-3.2 后只跑 stage=cd 一次)|
| audience overall_score | ≥ 9(P2-3.3 后含 Step 0 spot-check pre-gate) |
...
Pyramid 质量门**仅** critic 一处(P2-3.2 后):
...
| brainstorm (team) | `dispatch_author` | **关闭 brainstorm team**(P2-3.1 后),用 brainstorm yaml 里 `author_dispatch_preview` 直接派 author Stage C(无中间 critic Stage B,self-audit 已并入) |
| brainstorm (team) | `needs_self_revision` | P2-3.1 后新增 · brief self-audit 5 项有 fail / high severity;主线程展示 must_fix → 用户改 brief 或续 dialog → SendMessage 回 brainstorm |
...
| author | `dispatch_self_stage_d` | P2-3.2 后新增 · outline 批准后自走 Stage D,无中间 critic;主线程立即 Task(author, stage=D) |
...
| iloveppt-builder | `dispatch_audience` | Task(audience)— P2-3.3 后无中间 spot-check |
| iloveppt-builder | `hard_stop` | 展示 errors 给用户三选一 |
| audience | `delivered` | 交付 .pptx 给用户 |
| audience | `needs_author_rewrite` | Task(author) |
| audience | `needs_visual_redo` | Task(iloveppt-builder, mode=visual_redo)(P2-3.3 后:audience Step 0 spot-check fail 时也走此路径) |
...
**brainstorm 的 dispatch_author**(P2-3.1 后;含 brief self-audit 结果):
```yaml
agent: iloveppt-brainstorm
status: ok
...
**brainstorm 的 needs_self_revision**(P2-3.1 后;self-audit 5 项有 fail / high severity):
```yaml
agent: iloveppt-brainstorm
status: ok
...
**critic 必加字段**(P2-3.2 后只支持 stage=cd):
```yaml
...
**audience 必加字段**(P2-3.3 后含 Step 0 spot-check):
```yaml
...
**author 必加字段**(P2-3.2 后含 dispatch_self_stage_d):
```yaml
agent: iloveppt-author
status: ok
