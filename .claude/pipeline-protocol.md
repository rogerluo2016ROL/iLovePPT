...
4. **SendMessage 给 brainstorm**(首条消息只带三字段,不扩展):
   ...
   不传 `iloveppt_root`(2026-05-25 后已废弃 —— cwd 始终 = iLovePPT 仓库根,agent 直接用字面 `${CLAUDE_PROJECT_DIR}/.claude/skills/...`),也不传 `attached_files` / `user_profile_hints` / `previous_decks` 等扩展项 —— brainstorm 自己从 initial_request 解析、自己 Glob 找已有 deck/template。
