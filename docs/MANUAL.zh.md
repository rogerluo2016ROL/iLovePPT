...
| 已有审过的 brief.md / content.md,想跳到下一步 | 把 brief.md / content.md 路径告诉主线程,主线程根据状态派 author / iloveppt |
| 已有审过的 content.md,只想跑 build + 视觉 + audience | 直接说"按 content.md build 一份",主线程派 iloveppt mode=full |

**直接 `@agent-iloveppt` 但缺 content.md 会被 reject** —— iloveppt 不做 brief 解析、不写 content。
...
[critic Stage D · 14 项 + 4 维度判断性评审 content]
   ↓
   ├── pass / pass_with_notes → 派 iloveppt
   └── needs_revision → 你 cherry-pick → 派 author 改 content
   ↓
[Stage E · iloveppt 构建 .pptx + 17 项机械视觉 QA × ≤ 3 轮]
   ↓
...
content 批准 + critic Stage D pass 后,主线程派 **iloveppt** → **designer**(自动)→ **audience**。整段跑完返回:

```yaml
# iloveppt 返回
pptx_path: /abs/path/to/deck_v1.pptx
qa_rounds: 2
auto_md_edits:                          # iloveppt 自动改了哪几句(写到 .postbuild.md 副本,原文不动)
  - page: 5
...
**主线程会问**:"iloveppt 自动改了 1 处(page 5 action title 缩短)、designer 加了 8 处 icon、audience 给 9.2 分。要看 audience top 3 建议吗?接受交付 / 还想再改 / 回退某条 auto edit?"
...
**提案 / 路演 / 汇报 / 评审场合一律走金字塔** —— author / critic / iloveppt 三层 Pyramid 防线不接受"先放着"含糊过关。data_report / tutorial / catalog 等纯展示场景可在 brief 里说明,author 自检会软阻塞(可豁免附理由)。
...
重跑 Stage E iloveppt(outline 已批准,不用回 Stage C)
```

agent 会跳过 Stage A-D,从 Stage E iloveppt 开始重跑。
...
装好后让 agent 重跑 Stage E iloveppt,或自己跑 `build.py`。
...
| **iloveppt**(iloveppt,第 4 agent) | Stage E —— 接 content.md → Pyramid 自检 → md→JSON → build → 17 项机械视觉 QA。单次派发完成 |
| **iloveppt(Step 4 visual)**(第 5 agent) | Stage E.5 —— iloveppt 后自动跑,搜 iconify / Unsplash / brand assets,改 deck_plan.json 加 icon / hero / 装饰 |
...
| **review_needed** | Stage E iloveppt 输出的"需要人审"清单,agent 3 轮自检改不动的页 |
...
*适用流水线:5 agent + 1 旁路(brainstorm / author / critic / iloveppt / designer / audience · template-extractor)*
