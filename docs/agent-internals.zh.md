...
- [§ 1. 架构总览(15 min 读完)](#-1-架构总览15-min-读完)
- [§ 2. 能力库存(知识库 / 记忆 / 路由 / 仲裁)](#-2-能力库存知识库--记忆--路由--仲裁)
- [§ 3. 6 agent 详解(能力卡片 + 流程)](#-3-6-agent-详解能力卡片--流程)
- [§ 4. 架构决策(why)](#-4-架构决策why)
- [§ 5. 速查参考](#-5-速查参考)
...
### 1.2 Hybrid 二段论(Phase A team / Phase B subagent)
...
### 2.1 知识库 #1 · `library/visual-patterns`(hosted multimodal RAG)
...
### 2.3 记忆 #1 · `state.json`(per-agent 跨派发恢复)
...
### 2.4 记忆 #2 · markdown 产物链(brief → outline → content → deck_plan)
...
### 2.5 cherry-pick gate(主线程仲裁,2026-05-25 visual-patterns 5-agent extension 新增)
...
### 4.5 SSOT 在 helpers.py + content-writing.md schema
...
### 4.6 Hybrid 架构选择(brainstorm 留 team,其余转 subagent)
...
### 5.2 主线程派发表(从 pipeline-protocol.md §1 速查)
...
### 5.3 agent yaml return schema 全表(从 pipeline-protocol.md §4 速查)
