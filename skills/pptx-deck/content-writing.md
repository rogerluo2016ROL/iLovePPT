...
## 🆕 双模式字数表(v0.5.1 · brief.presentation_mode 控制)
...
1. **豁免某项** —— 必须附理由(例:"豁免第 3 项,理由:数据下周才有,本节先用占位")
   ...
   - builder Step 0 再跑 Pyramid 时,fail 项若被 author 豁免,会在 error 里附 `author_known_issues_note` 让用户最终决定
   - content-review 也会读 outline 末尾 unchecked 项,失败项若有理由可标注
2. **改某项** —— 收用户指令改 outline,改完重跑 7 项自检

不接受的回答(必须重问):
- "先放着" / "后面再改" / "不管它" / "差不多就行" / 沉默不回

这是 v0.5.1 软阻塞,不阻断用户节奏,但留 audit 痕迹。
...
footer_meta:                            # 🆕 v0.5.1:author Stage C 出 outline 时**默认填**,用户可改
  ...
  version: v1.0                         # 默认 v1.0,跟 author state.iteration 同步(v2 → v2.0)
---
