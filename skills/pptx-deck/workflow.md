...
1. `prs.save("/tmp/iloveppt_render/current.pptx")`
...
输出：`examples/sample_output.pptx`（12 页）+ 渲染 PNG 到 `/tmp/iloveppt_render/`。
...
2. Claude 用 Read tool 逐张读取 `/tmp/iloveppt_render/page_NN.png`
...
render_one_slide(prs, idx, f"/tmp/iloveppt_render/page_{idx:02d}.png")
```
